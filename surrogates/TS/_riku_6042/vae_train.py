...
if __name__ == "__main__":
    ...
    VAE = vae.VarAutoEncoder(gauges=[5832, 6042], model_name='vae_riku')
  ...
  # identify the epochs where the loss is minimum
    ...
    # # training kl loss plot
    # kl_loss = np.load('_output/vae_riku_kl_loss_k{:d}.npy'.format(kfold2use))
    # s0 = 2
    # fig, ax = plt.subplots(figsize=(s0 * 6, s0 * 4))
    # line0, = ax.semilogy(kl_loss[1:],color='red',linewidth=1, label='KLD loss')
    # ax.grid(which='minor', alpha=0.05)
    # ax.grid(which='major', alpha=0.1)
    # ax.xaxis.set_ticks(np.arange(0, nepochs2use, 500))
    # ax.set_ylabel('loss')
    ...
    # fname = str(1024) + '_vae_KLD_losses_k{:d}.png'.format(kfold2use)
    # save_fname = os.path.join('_output', fname)
    # fig.savefig(save_fname)
