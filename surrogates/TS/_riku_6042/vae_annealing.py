...
class VarAutoEncoder():
    ...
    def train_vae(self,vaeruns=100,
        ...
        annealing_agent = Annealer(total_steps=1000, shape='cosine',baseline=0.4)  # initialising annealing agent
        
        # epochs
        train_loss_array = np.zeros(nepochs+1)
        test_loss_array = np.zeros(nepochs+1)
        kl_loss_array = np.zeros(nepochs+1)
        ...
        for epoch in range(1, nepochs+1):
            ...
            for k in range(nbatches_train):
                ...
                bce,kld = loss_func(outputs[0][:,0,:], data0[:,-1,:], outputs[1], outputs[2]) #batch level loss #output gaguge is last gauge       
                kld = annealing_agent(kld)  #applying annealing function to KL divergence loss

                (bce+kld).backward() #computes dloss/dx for every parameter x ie backpropagation
                ...
                annealing_agent.step() #increment annealing step
                
                train_loss += bce.item() #cumalitive epoch loss (MSE loss)
                kl_loss += kld.item() #cumalitive epoch loss (KL loss)
            ...
            avg_KL_loss = kl_loss/nbatches_train
            kl_loss_array[epoch] = avg_KL_loss
            ...
            fname = '{:s}_kl_loss_k{:d}.npy'\
                    .format(model_name, kfold)
            save_fname = os.path.join(output_path, fname)
            np.save(save_fname, kl_loss_array)
    ...
    def predict_dataset(self,epoch,kfold, device='cpu'):
        ...
        z_all = np.zeros((vaeruns,ndata, 25)) #only one output gauge
