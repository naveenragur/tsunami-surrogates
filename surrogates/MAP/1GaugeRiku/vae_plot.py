...
if __name__ == "__main__":
    ...
    for epoch in epochs2use:
        ...
        title = "Max flood depth gauge using input {:3d}, fold {:d} ".format(gauge, kfold2use)

        ax.set_title(title, fontsize=10)

        fname = r'{:s}_predvobs_inputg{:03d}_k{:d}_e{:04d}.{:s}' \
        ...
        for h,hist_name in enumerate(hist_name_list):
            ...
            # plots begin
            west, south, east, north = (141.6,38.94,141.7,39.04)
            ...
            if not os.path.exists('_data/fgmax003_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax003_base.tif')
                cx.add_basemap(mx, source='_data/fgmax003_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(mx, source='_data/fgmax003_base.tif', crs='EPSG:4326')
            mx.scatter(fgmaxloc[TP,0], fgmaxloc[TP,1],s=0.5,c='b',alpha=0.6)
            mx.scatter(fgmaxloc[FP,0], fgmaxloc[FP,1],s=0.5,c='r',alpha=0.6)
            mx.text(141.64,38.98,s='TPR(TP/P):' + str(TPR), fontsize=12,color='white')
            mx.text(141.64,38.97,s='FPR(FP/N):' + str(FPR), fontsize=12,color='white')
            mx.text(141.64,38.96,s='Accuracy(True): ' + str(AC), fontsize=12,color='white')
            mx.text(141.64,38.95,s='Precision: ' + str(PR), fontsize=12,color='white')
            ...
            if not os.path.exists('_data/fgmax003_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax003_base.tif')
                cx.add_basemap(nx, source='_data/fgmax003_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(nx, source='_data/fgmax003_base.tif', crs='EPSG:4326')
            nx.scatter(fgmaxloc[FN,0], fgmaxloc[FN,1],s=0.5,c='m',alpha=0.6)
            nx.scatter(fgmaxloc[TN,0], fgmaxloc[TN,1],s=0.5,c='g',alpha=0.6)
            nx.text(141.64,38.98,s='TNR(TN/N): ' + str(TNR), fontsize=12,color='white')
            nx.text(141.64,38.97,s='FNR(FN/P): ' + str(FNR), fontsize=12,color='white')
            nx.text(141.64,38.96,s='Error(False): ' + str(ER), fontsize=12,color='white')
            ...
            if not os.path.exists('_data/fgmax003_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax003_base.tif')
                cx.add_basemap(ox, source='_data/fgmax003_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(ox, source='_data/fgmax003_base.tif', crs='EPSG:4326')   

            ox.scatter(fgmaxloc[P,0], fgmaxloc[P,1], s=0.5, c=pred_mean_hist[P], cmap=cmap, alpha=0.6)
            ox.text(141.64,38.98,s='RSquare:' + str(round(r2_score(obs_hist, pred_mean_hist), 3)), fontsize=12,color='white')
            ox.text(141.64,38.97,s='G:' + str(round(Gfit(obs_hist, pred_mean_hist), 3)), fontsize=12,color='white')
            ...
            ax.cla()
            ax.set_xlim(141.5221845938366414,141.7943840913690963)
            ax.set_ylim(38.8951267213683849,39.0875910125529487)

            fig_title = "Event {:s} vs 2011 TTJS Flood Extent".format(hist_name)
            ax.set_title(fig_title, fontsize=15)
            cx.add_basemap(ax, source='_data/fgmax003_Tohoku_base.tif', crs='EPSG:4326')   
            ax.scatter(fgmaxloc[P,0], fgmaxloc[P,1], s=0.5, c=pred_mean_hist[P],cmap=cmap,
                        alpha=.18, marker=',', edgecolors='none')
            img = plt.imread('_plots/depth.png') 
            im = OffsetImage(img, zoom=.45)
            ab = AnnotationBbox(im, (1, 0), xycoords='axes fraction', box_alignment=(1.1,-0.1))
            ax.add_artist(ab)
        ...
        for runno in test_runno[-5:]:
            ...
            fname = 'vae_depthsmap_test{:4d}_k{:d}_e{:04d}.{:s}'.format(runno,kfold2use,epoch,fmt)
            fname = os.path.join('_plots', fname)
