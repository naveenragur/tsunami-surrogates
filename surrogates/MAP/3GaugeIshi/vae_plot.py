...
if __name__ == "__main__":
    ...
    for epoch in epochs2use:
        ...
        for h,hist_name in enumerate(hist_name_list):
            ...
            # plots begin        
            west, south, east, north = (141.10,38.32,141.42,38.52)
            ...
            if not os.path.exists('_data/fgmax002_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax002_base.tif')
                cx.add_basemap(mx, source='_data/fgmax002_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(mx, source='_data/fgmax002_base.tif', crs='EPSG:4326')
            mx.scatter(fgmaxloc[TP,0], fgmaxloc[TP,1],s=0.5,c='b',alpha=0.6)
            mx.scatter(fgmaxloc[FP,0], fgmaxloc[FP,1],s=0.5,c='r',alpha=0.6)
            mx.text(141.15,38.5,s='TPR(TP/P):' + str(TPR), fontsize=12,color='white')
            mx.text(141.15,38.48,s='FPR(FP/N):' + str(FPR), fontsize=12,color='white')
            mx.text(141.15,38.45,s='Accuracy(True): ' + str(AC), fontsize=12,color='white')
            mx.text(141.15,38.42,s='Precision: ' + str(PR), fontsize=12,color='white')
            ...
            if not os.path.exists('_data/fgmax002_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax002_base.tif')
                cx.add_basemap(nx, source='_data/fgmax002_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(nx, source='_data/fgmax002_base.tif', crs='EPSG:4326')
            nx.scatter(fgmaxloc[FN,0], fgmaxloc[FN,1],s=0.5,c='m',alpha=0.6)
            nx.scatter(fgmaxloc[TN,0], fgmaxloc[TN,1],s=0.5,c='g',alpha=0.6)
            nx.text(141.15,38.5,s='TNR(TN/N): ' + str(TNR), fontsize=12,color='white')
            nx.text(141.15,38.47,s='FNR(FN/P): ' + str(FNR), fontsize=12,color='white')
            nx.text(141.15,38.45,s='Error(False): ' + str(ER), fontsize=12,color='white')
            ...
            if not os.path.exists('_data/fgmax002_base.tif'):
                img, ext = cx.bounds2raster(west,south,east,north,ll=True,source=cx.providers.Esri.WorldImagery,path='_data/fgmax002_base.tif')
                cx.add_basemap(ox, source='_data/fgmax002_base.tif', crs='EPSG:4326')
            else:
                cx.add_basemap(ox, source='_data/fgmax002_base.tif', crs='EPSG:4326')   

            ox.scatter(fgmaxloc[P,0], fgmaxloc[P,1], s=0.5, c=pred_mean_hist[P], cmap=cmap, alpha=0.6)
            ox.text(141.15,38.5,s='RSquare:' + str(round(r2_score(obs_hist, pred_mean_hist), 3)), fontsize=12,color='white')
            ox.text(141.15,38.47,s='G:' + str(round(Gfit(obs_hist, pred_mean_hist), 3)), fontsize=12,color='white')
            ...
            ax.cla()
            ax.set_xlim(141.0928193370000088,141.3888844479999989)
            ax.set_ylim(38.3206155129999999,38.5080024640000005)

            fig_title = "Event {:s} vs 2011 TTJS Flood Extent".format(hist_name)
            ax.set_title(fig_title, fontsize=15)
            cx.add_basemap(ax, source='_data/fgmax002_Tohoku_base.tif', crs='EPSG:4326')
